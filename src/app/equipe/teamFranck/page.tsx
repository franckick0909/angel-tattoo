...
export default function FranckPage() {
  const router = useRouter();
  const member = teamData.team.find((m) => m.id === "teamFranck");
    ...
    <section className="relative w-full min-h-screen py-36 px-4 md:px-8 lg:px-16 xl:px-20">
      ...
      <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-20">
        <div className="w-full h-full flex flex-col items-start justify-start gap-4 overflow-hidden">
          <GradualSpacing
            className="text-7xl md:text-8xl lg:text-[112px] xl:text-[200px] text-white uppercase flex flex-wrap items-center justify-start w-full tracking-tight font-black"
            text={member.name}
          />
