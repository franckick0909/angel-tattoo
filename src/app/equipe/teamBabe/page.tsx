...
export default function BabePage() {
  const router = useRouter();
  const member = teamData.team.find((m) => m.id === "teamBabe");
    ...
    <section className="relative w-full min-h-screen py-36 px-4 md:px-8 lg:px-16 xl:px-20">
      ...
      <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-20">
        <div className="w-full h-full flex flex-col items-start justify-start gap-4 overflow-hidden">
          ...
          <motion.div initial={{ scaleY: 0.5, opacity: 0, x: -30 }} whileInView={{ scaleY: 1, opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.3 }} className="py-16 w-full">
            {member.description[0].demande && (
              <Link
                href="/#Contact"
                onClick={handleContactClick}
                className="text-slate-950 text-xl md:text-2xl hover:text-white ease-in-out bg-white hover:bg-transparent border-2 border-white hover:border-custom-green  py-6 transition-colors duration-300 my-14 px-14 md:px-24 w-full"
              >
                {member.description[0].demande}
              </Link>
            )}
